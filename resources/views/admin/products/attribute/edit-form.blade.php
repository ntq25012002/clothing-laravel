    ...
    <div class="card-body">
        <form action="" method="post" enctype="multipart/form-data"class="form-horizontal col-md-6">
            ...
            <div class="form-group attr {{$attr->name == 'color' ? 'active' : ''}} " id="color" >
                ...
                <input type="color" name="color" value="{{$attr->name == 'color' ? $attr->value : ''}}"id="input-color" >
            </div>
            <div class="form-group attr {{$attr->name == 'size' ? 'active':''}}" id="size">
                <label for="">Giá trị</label>
                <input type="text" name="size" value="{{$attr->name == 'size' ? $attr->value : ''}}" id="input-size"placeholder="Kích thước" class="form-control">
            </div>
            <div>
