                ...
                @endif
                    ...
                    <div class="d-flex">
                        <div class="col-sm-6">
                            ...
                            <div class="form-group  px-0 image-feature">
                                <label class="form-label" for="">Ảnh đại diện</label>
                                <input class="form-control mb-2" id="formFile" type="file" onchange="previewImage()"
                                    name="feature_image" value="{{ old('feature_image') }}">
                                <div class="image-preview">
                                    <img src="{{asset('storage/product/'.$product['feature_image'])}}" id="img-preview" alt="" >
                                </div>
                                @error('feature_image')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="form-group image-details ">
                                <label class="form-label" for="">Ảnh chi tiết</label>
                                <input type="file" class="form-control mb-2" id="imageDetails" multiple name="images[]">
                                <div class="d-flex flex-wrap imageDetailsContainer">
                                    @foreach ($product->images as $imageDetailItem)
                                        <img src="{{asset('storage/product/'.$imageDetailItem->image)}}" alt="" class="product-image-detail">
                                    @endforeach
                                </div>
                                @error('images')
                                    <span class="text-danger">{{ $message }}</span>
                    ...
                    <div class="d-flex justify-content-end mt-3 col-12">
                        ...
                        <div data-url="{{ route('admin.product.edit',['id' =>$product['id'] ])}}" name="submit" class="btn btn-primary btn-edit">Cập nhật sản phẩm</div>
                    </div>
                </div>
            </div>
        </form>
