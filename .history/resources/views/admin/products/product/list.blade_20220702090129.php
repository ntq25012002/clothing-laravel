        ...
        <div class="form-group">
            <select name="category" id="" class="form-control-sm form-control form-select">
                ...
                @foreach ($categories as $category)
                    <option value="{{$category['id']}}" {{$category['id'] == $search_category ? 'selected' : ''}}>{{$category['name']}}</option>
                @endforeach
            </select>
        </div>
        ...
        <table class="table table-hover mt-3">
            <thead>
                <tr>
                    ...
                    <th>Ảnh </span></th>
                    <th>Sản phẩm <span> ({{ $categories->count() }})</span></th>
                    <th>Số lượng </span></th>
                    <th>Giá </th>
                    <th>Danh mục </th>
                    <th>Slug </th>
            ...
            <tbody>
                ...
                @foreach ($products as $pr)
                    <tr>
                        ...
                        <td>
                            <img src="{{asset('product/'.$pr['feature_image'])}}" alt="Ảnh sản phẩm">
                        </td>
                        <td>
                            {{$pr['name']}}
                        </td>
                        ...
                        <td>
                            <a href="{{ route('admin.category.edit', $pr['id']) }}">
                                <i class="fas fa-pen-to-square"></i>
                        </td>
                        </a>
                        <td>
                            <a href="{{ route('admin.category.delete', $category['id']) }}" class="text-danger btn-delete">
