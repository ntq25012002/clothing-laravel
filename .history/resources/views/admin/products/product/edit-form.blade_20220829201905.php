        ...
        <form action="" method="post" enctype="multipart/form-data" id="form-add" class="form mb-2">
            ...
            <div class="card-body">
                ...
                @endif
                    ...
                    <input type="text" name="imageOlds" class="image-olds" value="{{$prdImgOlds}}" hidden>
                            ...
                            <div>
                                ...
                                <div>
                                    @foreach ($sizeAttrs as $item)
                                        <input type="checkbox" name="attrs[]" id="size{{$item->id}}" {{$product->attributes->contains($item->id) ? 'checked' : ''}}
                                            value="{{ old('color-attr') ?? $item->id}}" style="font-size:20px"> 
                                        <label for="size{{$item->id}}" class="ml-1 mr-2">{{$item->value}}</label>
                                    @endforeach
                                </div>
                            </div>
                            <div>
                                ...
                                <div>
                                    @foreach ($colorAttrs as $item)
                                        <input type="checkbox" name="attrs[]" id="color{{$item->id}}" {{$product->attributes->contains($item->id) ? 'checked' : ''}}
                                        value="{{ old('color-attr') ?? $item->id }}"> 
                                        <label for="color{{$item->id}}" class="ml-1 mr-2">
                                            <i class="nav-icon fa-solid fa-shirt" style="color: {{ $item->value}}; font-size: 22px"></i>
                                        </label>
                                    @endforeach
                                </div>
                            </div>
                ...
                <div>
                    <div class=" px-0 image-feature">
                        ...
                        <div class="image-preview">
                            <img src="{{asset($product->feature_image)}}" id="img-preview" alt="" width="90%">
                        </div>
                        @error('feature_image')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
