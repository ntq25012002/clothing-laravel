    ...
    public function postCheckOut(OrderRequest $request) {
        ...
        try {
            ...
            foreach (session('cartQuantity') as $key => $value) {
                ...
                $product->quantity = $product->quantity - $value['quanitity'];
                $this->products->save();
                // $this->products->where('id',$key)->update('quantity','quanitity' - $value['quantity']);
                dd($product);
            }
            ...
            session()->forget(['myCart','total','totalItemPrd','discount','idDisc','codeDisc']);
            return redirect()->route('complete-order')->with('code', $order->code);
