...
class CartController extends Controller
{
    ...
    public function updateCart(Request $request) {
        ...
        if($request->discount != null) {
            $code = [
                'code-disc' => $request->discount
            ];
            session()->put('codeDisc',$code);
            $discount = $this->discounts->where('name',$request->discount)
                                        ->where([
            ...
            if(!empty($discount)) {
                ...
                session()->put('discount', $disc);
                if($type == 1) {
                    $total = session()->get('total') - session()->get('total')*$value/100;
                }else if($type == 2) {
                    $total = (session()->get('total') - $discount['value']) > 0 ?? 0;
                }
                // session()->put('total', $total);
                return response()->json([
                    'status' => '200',
    ...
    public function postCheckOut(OrderRequest $request) {
        ...
        try {
            ...
            if($discount['type'] == 1) {
                $total = session()->get('total') - session()->get('total')*$discount['value']/100;
            ...
            $id = session()->get('idDisc');
            $this->discounts->where('id',$id)->update([
                'status' => 2
            ]);

            foreach (session('cartQuantity') as $key => $value) {
                $product = $this->products->where('id',$key)->first();

                // dd($product->quantity,$value);
                $quantity = $product->quantity - $value['quantity'];
                // $product->save();
                $this->products->where('id',$key)->update([
                    'quantity' => $quantity
                ]);
            }
            ...
            if(!empty($order)) {
                ...
                if(!empty(session()->get('discount'))) {
                    session()->forget(['discount','idDisc','codeDisc']);
                }
                session()->forget(['myCart','total','totalItemPrd','idDisc','codeDisc']);
                return redirect()->route('complete-order')->with('code', $order->code);
            }else{
                return redirect()->route('checkout')->with('err','Lỗi đặt hàng');
            } 

            session()->forget(['myCart','discount','total','totalItemPrd','idDisc','codeDisc']);
            return redirect()->route('complete-order')->with('code', $order->code);
