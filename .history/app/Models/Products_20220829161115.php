...
class Products extends Model
{
    ...
    public function loadList($params = [],$filters= [],$categoryId = null) {
    ...
    public function loadListPaginate($params = [],$categoryId = null,$lim = 10) {
        $query = $this->where('status',1)->orderBy('id','desc');
        if(!empty($params)) {
            $query = $query->where($params);
        }
    ...
    public function similarProducts($categoryId,$id) {
        $query = $this->where('category_id', $categoryId)->where('id','<>',$id);
        $list = $query->get();
        return $list;
    }
